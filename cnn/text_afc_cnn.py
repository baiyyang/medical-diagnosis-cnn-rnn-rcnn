...
class AFCTextCNN(object):
    ...
    def __init__(self, sequence_length, num_classes, vocab_size, embedding_size, filter_sizes, num_filters,
        ...
        # Create a convolution + all_features_concat layer for each filter size
        pool_ans = []
        ...
        filter_length = 0
        for pool in pool_ans:
            filter_length += int(pool.shape[1])
        print("filter length: {}".format(filter_length))

        self.h_pool = tf.concat(pool_ans, axis=1)
        print("h_pool shape: {}".format(self.h_pool.shape))

        self.h_pool_flat = tf.reshape(self.h_pool, shape=[-1, filter_length])
        ...
        with tf.name_scope('output'):
            W = tf.get_variable('W', shape=[filter_length, num_classes],
